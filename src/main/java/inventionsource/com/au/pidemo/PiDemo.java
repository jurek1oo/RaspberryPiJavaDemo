 ...
 */
    ...
    public static void main(String args[]) throws Exception {
        ...
        try {
            ...
            //
            GpioPinDigitalOutput outYellow = gpio.provisionDigitalOutputPin(RaspiPin.GPIO_27,"outYellowPin27",PinState.LOW);
            outYellow.clearProperties();
            outYellow.setShutdownOptions(true, PinState.LOW, PinPullResistance.OFF);
            outYellow.low();
            
            GpioPinDigitalOutput outGreen = gpio.provisionDigitalOutputPin(RaspiPin.GPIO_25,"outGreenPin25",PinState.LOW);
            ...
            GpioPinDigitalInput inBlue = gpio.provisionDigitalInputPin(RaspiPin.GPIO_22,"inPin22", PinPullResistance.PULL_UP);
            ...
            log.info("------------------------- Output/Input Pins setup done. --------------------------------------");
            ...
            inBlue.addListener(new GpioPinListenerDigital() {
                 ...
                 public void handleGpioPinDigitalStateChangeEvent(GpioPinDigitalStateChangeEvent event) {
                    ...
                    String blueStatusStamp = (DateTimeFormatter.ofPattern("yyyy-MM-dd_HH:mm:ss").format(LocalDateTime.now())) +
                            "-" + blueStatus ; 
                    outGreen.low();
                    outYellow.low();
                    log.info("PinDigitalStateChange now blueStatusStamp: "+ blueStatusStamp);
                       try {
                            ...
                            if ((blueStatus != "LOW" && blueStatus != "HIGH") ){
                                log.error("PinDigitalStateChange Error somewhere. blueStatus != LOW && blueStatus != HIGH blueStatus: " );
                                ToggleLed(outYellow, 1000, 10);
                            }
                        } catch (Exception ex) {
                           // Exception toggle outYellow led on error
                            log.error("Exception Error PinDigitalStateChange blueStatus : " + blueStatus, ex.getMessage());
    ...
    public static void ToggleLed(GpioPinDigitalOutput outPin, long mills , int count){
        log.error("outPin: " + outPin.getName()+ " toggle mmills: "+ mills + " count: " + count);
        try {
            ...
            log.error("Error in toggle: " + outPin.getName()+ "  mills: "+ mills + " count: " + count, e);
        }
    }
}
